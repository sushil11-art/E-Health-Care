...
def disease(request):
    ...
    if search_term == None:
        search_term = ""
    diseases = Disease1.objects.annotate(fullname=Concat('doctor__user__first_name', Value(' '), 'doctor__user__last_name')).filter(Q(name__icontains=search_term) | Q(
        doctor__user__first_name__icontains=search_term) | Q(doctor__user__last_name__icontains=search_term) | Q(fullname__icontains=search_term)).order_by('-id')
...
def edit_doctor(request, pk):
    ...
    mydict = {'userForm': userForm,
              'doctor_add_form': doctor_add_form, 'doctor': doctor}
    if request.method == 'POST':
...
def patients_list(request):
    ...
    if search_term == None:
        search_term = ""
    patients = Profile.objects.filter(user_id__in=users).filter(Q(name__icontains=search_term) | Q(
        user__email__icontains=search_term) | Q(address__icontains=search_term))
    context = {
        'patients': patients,
    }
