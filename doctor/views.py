...
def prescription(request):
    if request.method == "GET":
        ...
        send_mail("Thank you for using E-Health Care services!!", "Get Well soon!!",
...
def dashboard_doctor(request):
    ...
    if search_term is not None:
        search_term = search_term.ltrim().rtrim()
    contex = {}
    from doctor.models import DoctorInfo
...
def appointment(request):
    ...
    appointments = AppointmentDetails.objects.filter(
        create_by=request.user.doctorinfo)
    context = {
...
def edit_appointment(request, pk):
    try:
        appointment = AppointmentDetails.objects.get(
            id=pk, create_by=request.user.doctorinfo)
    except:
        return redirect('appointment')
    if request.method == 'POST':
        appointment_edit_form = AddAppointmentForm(
            request.POST or None, request.FILES, instance=appointment)
